		...
		2. <%@ include %> ==> 현재 페이지에 다른 문서(JSP, HTML)를 가져와서 내용을
					...
					- include 지시어를 사용한 JSP 페이지가 컴파일 되는 과정에서
					  include 되는 JSP 페이지의 소스 내용을 그대로 포함해서
					    컴파일을 진행함.
					- 즉, 복사 & 붙이기 방식으로 두 개의 파일이 하나의 파일로
					    구성이 된 후 같이 컴파일이 됨.
		3. <%@ taglib %> ==> 사용할 태그 라이브러리 지정
					- EL / JSTL 언어 사용시 적영되는 디렉티브(추후 수업 진행 예정)
...
-			<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	<%-- 
	 	...
	 	1. 스크립틀릿 : 가장 일반적으로 JSP 페이지에서 많이 쓰이는 스크립트 요소.
	 				...
	 				형식) <%
	 					자바 코드;
	 				%>
	 	2. 표현식 : 일반적으로 JSP 페이지에서 자바의 System.out.println()과 유사하게
	 			사용됨. 데이터를 출력할 때 주로 사용됨.
	 			형식) <%=변수명, 수식%>
	 			
	 	3. 선언문 : 일반적으로 JSP 페이지에서 자바의 멤버변수 또는 멤버메서드를
	 			선언할 때 사용됨.
	 			형식) <%!
	 				변수 선언 및 메서드 선언
	 			%>
	...
	<%-- JSP 표현식 : 변수으 ㅣ값 또는 메서드의 결과값을 출력하는 공간 --%>
